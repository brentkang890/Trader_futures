...
right_x = int(w0.76)
crops = [
img_cv[int(h0.02):int(h0.98), right_x:w].copy(),
img_cv[int(h0.06):int(h0.94), int(w0.7):w].copy()
]
vals = None
for crop in crops:
try:
...
if not vals:
return {}
ys = np.linspace(int(h0.02), int(h0.98), num=len(vals)).astype(int).tolist()
prices = vals
...
h,w,_ = img_cv.shape
top = int(h0.06); bottom = int(h0.94)
left = int(w0.06); right = int(w0.94)
