...
if poi:
    if poi.get("demand"):
        ...
        smc.append(f"📍Demand POI near: {lvl}")
    elif poi.get("supply"):
        lvl = poi["supply"][0]["level"]
        smc.append(f"📍Supply POI near: {lvl}")

fvg = d.get("fvg", {})
if fvg:
    if fvg.get("bullish_fvg"):
        smc.append("🧲 Bullish FVG nearby")
    elif fvg.get("bearish_fvg"):
        smc.append("🧲 Bearish FVG nearby")

ob = d.get("order_blocks", {})
if ob and (ob.get("bullish_ob") or ob.get("bearish_ob")):
    smc.append("🏛 Order Block detected")

hs = d.get("patterns", {})
if hs:
    smc.append(f"📐 Pattern: {hs.get('pattern')}")

apx = d.get("apex")
if apx:
    smc.append("🔻 Apex compression (break potential)")

if smc:
    resp += "\n\nSMC Highlights:\n" + "\n".join(f"• {x}" for x in smc[:3])
